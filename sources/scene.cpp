...
Scene::Scene(const Config& config) {
    ...
    for (auto object_config: config.objects_config) {
        ...
        if (object_config.material_name == "red_diffuse") object->setMaterial(red_mat);
        else if (object_config.material_name == "green_diffuse") object->setMaterial(green_mat);
        else object->setMaterial(grey_mat);
